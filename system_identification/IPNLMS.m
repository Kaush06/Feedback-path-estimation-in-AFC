...
w_est_IPNLMS = 0.00001*ones(1,length(w));      %Estimated Weight Vector IPNLMS
    ...
    del_IPNLMS=((1-alpha)/2*length(w))*delta;

    zeeta = ( L/(L-sqrt(L)) ) * (1-( norm(w_est_IPNLMS,1)/(sqrt(L)*norm(w_est_IPNLMS,2))+0.0001 ) ) % Sparsity measure
  
     
     for j=1:length(w)
         F1(j) = (((1-0.5*zeeta )/L)*((1-alpha)/2*L) );
         F2(j) =  ((1+0.5*zeeta )/L) * ( ((1+alpha) *abs(w_est_IPNLMS(j)))/(2*norm(w_est_IPNLMS,1)+0.1) );
         F(j) = F1(j) + F2(j);
     end
      phi_IPNLMS=diag(F);
